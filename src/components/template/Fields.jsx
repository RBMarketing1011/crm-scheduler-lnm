...
export function CheckboxField ({ label, type, className, value, onChange, ...props })
{
  let id = useId()
  ...
  return (
    <div className={ className }>
      { label && <Label id={ id }>{ label }</Label> }
      <input style={ { width: '15px', height: '20px' } } id={ id } type={ type } { ...props } className={ formClasses } value={ value } onChange={ onChange } />
    </div>
  )
}